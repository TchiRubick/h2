      ...
      <div className='w-full'>
        <table className='w-full border-4 border-white'>
          <thead className='w-full  text-sm text-white '>
            <tr className='grid h-fit w-full grid-cols-7 gap-3 rounded-xl border border-solid border-white py-4 text-center '>
              <th>Barcode</th>
              <th>Name</th>
              <th>Cost</th>
              <th>Price</th>
          ...
          <tbody className='mb-auto mt-4 grid w-full gap-y-4'>
            {data?.length === 0 ? (
              ...
              data?.map((stock) => (
                ...
                >
                  <tr className='grid w-full grid-cols-7 gap-3 text-center'>
                    <td>{stock.barcode}</td>
                    <td>{stock.name}</td>
